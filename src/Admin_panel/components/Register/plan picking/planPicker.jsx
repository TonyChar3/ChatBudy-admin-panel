...
const PlanPickerPage = () => {
    ...
    const RegisterUser = async() => {
        try{
            ...
            if(register_user_info === null){
                setModalOpen(true);
                setModalErrorMode(true);
                setModalMsg("Please enter register or log in again, you need your credentials set up again :(");
            ...
            if(plan_picked === 'plus'){
                ...
                if(create){
                    await updateProfile(auth.currentUser, { displayName: register_user_info.user_name });
                    await checkoutPlusPlan('new_client', auth.currentUser.accessToken, {...register_user_info, password: "none", plan: plan_picked });
    ...
    const SignedInUser = async() => {
        try{
            ...
            // login w/ firebase
            const auth_user = await signInWithEmailAndPassword(auth, login_user_info.email, login_user_info.password)
            ...
            > 
                ...
                <div className="w-[80%] h-auto flex flex-col justify-center items-center">
                    <div className="w-full text-center my-1 text-[#A881D4]">
                        <h2 className="text-4xl font-light md:text-3xl lg:text-5xl">Choose your plan</h2>
                    ...
                    <div className="w-full h-auto justify-center justify-center items-center">
                        <div onClick={() => setPlanPicked('standard')} className={`w-[90%] lg:w-[45%] mx-auto mt-2 p-2 lg:p-3 flex flex-row items-center rounded-xl border-2 cursor-pointer active:scale-[0.90] transform-all ease duration-300
                        ${plan_picked.length > 0 && plan_picked === 'standard'? 'border-[#6C2E9C] text-[#A881D4]' : plan_picked.length > 0? 'border-[#F5F3EF] text-[#F5F3EF]' : 'border-[#6C2E9C] text-[#A881D4]'}`}>
                            <h3 className="lg:text-xl">Standard</h3>
                            <span className="mx-auto lg:text-lg">Free*</span>
                            <div className={`w-[20px] h-[20px] rounded-full
                            ${plan_picked.length > 0 && plan_picked === 'standard'? 'bg-[#50C878]' : plan_picked.length > 0? 'bg-[#F5F3EF]' : 'bg-[#F5F3EF]'}`}></div>{/**Circle */}
                        </div>
                        <div onClick={() => setPlanPicked('plus')} className={`w-[90%] lg:w-[45%] mx-auto mt-6 lg:mt-8 mb-4 p-2 lg:p-3 flex flex-row items-center rounded-xl border-2 text-[#A881D4] cursor-pointer active:scale-[0.90] transform-all ease duration-300
                        ${plan_picked.length > 0 && plan_picked === 'plus'? 'border-[#6C2E9C] text-[#A881D4]' : plan_picked.length > 0? 'border-[#F5F3EF] text-[#F5F3EF]' : 'border-[#6C2E9C] text-[#A881D4]'}`}>
                            <h3 className="lg:text-xl">Plus +</h3>
                            <span className="mx-auto lg:text-lg">15$/month</span>
                            <div className={`w-[20px] h-[20px] rounded-full
                            ${plan_picked.length > 0 && plan_picked === 'plus'? 'bg-[#50C878]' : plan_picked.length > 0? 'bg-[#F5F3EF]' : 'bg-[#F5F3EF]'}`}></div>{/**Circle */}
                        </div>
                    </div>
                    {
                        plan_picking_state === 'new_client'?
                        <>
                            <button onClick={RegisterUser} className="bg-[#6C2E9C] p-2 text-lg text-white font-light rounded-xl w-[30%] lg:w-[10%] text-center mb-3 lg:text-xl">
                                Next
                            </button>
                        </>
                        :
                        <>
                            <button onClick={SignedInUser} className="bg-[#6C2E9C] p-2 text-lg text-white font-light rounded-xl w-[30%] lg:w-[10%] text-center mb-3 lg:text-xl">
                                Next
                            </button>
                        </>
                    }
