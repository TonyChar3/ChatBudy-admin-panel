...
const VisitorCard = ({ id, name, email, browser, country, time, visitor_mode, unread_chat_count }) => {
        ...
        <>
            <div className="w-[95%] md:w-[85%] relative flex flex-row justify-around p-4 md:p-4 lg:p-4 my-4 border-[1px] border-[#6C2E9C] rounded-xl shadow-custom-shadow-input bg-white">
                <div className={`${show_full_name && isNameTruncated? 'w-full':'w-1/2'} flex flex-row justify-start items-center z-5`}>
                    ...
                    <div className={`relative ${show_full_name && isNameTruncated? 'w-full' : 'lg:w-1/2 w-full'}`}>
                        ...
                        <span className={`${unread_chat_count === 0? 'hidden' : 'bg-red-500 text-white'} absolute top-0 left-[65%] w-[25px] h-auto text-center rounded-full`}>{unread_chat_count}</span>
                    </div>
                ...
                <div className={`absolute z-10 right-[12%] top-1 lg:top-4 p-2 flex flex-row items-center text-sm text-center
                ${visitor_mode === 'live-chat'? 'justify-between w-[30%] lg:w-[10%]' : 'justify-center w-[18%] lg:w-[8%]'}  
                ${open_actions? '':'scale-0'} border-[1px] border-[#A881D4] rounded-3xl bg-[#F5F3EF] duration-300`}>
                    ...
                    <i onClick={() => openChatRoom()} className={`${visitor_mode === 'live-chat'? '' : 'hidden'} fa-sharp fa-light fa-comment text-2xl lg:text-3xl text-[#A881D4] active:scale-[0.90] cursor-pointer`}></i>
                </div>
            </div>
