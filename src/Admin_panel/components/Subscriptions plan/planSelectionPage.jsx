...
import { checkoutPlusPlan } from '../../../context/utils/manageAuth';
...
const PlanSelectionPage = () => {
    ...
    const { 
        ...
        setModalMsg, 
        setModalErrorMode,
        setShowLoader } = UserAuth()

    const PlusCheckout = async() => {
        try{
            setShowLoader(true);
            const checkout = await checkoutPlusPlan('logged_client', user.accessToken);
            if(checkout.error){
                setShowLoader(false);
                setModalOpen(true);
                ...
                >
                    ...
                    <div className="w-full h-full flex flex-col lg:flex-row justify-center items-center">
                        ...
                        <div className="w-[90%] lg:w-[35%] h-[600px] p-2 flex flex-col justify-around items-center border-2 border-[#6C2E9C] rounded-xl">
                            ...
                            <div onClick={PlusCheckout} className="w-[55%] border-2 border-[#6C2E9C] text-[#6C2E9C] text-center p-2 rounded-lg my-1 text-xl active:scale-[0.90] hover:text-[#A881D4] hover:border-[#A881D4] hover:scale-[1.01] transition-transform ease duration-100 cursor-pointer">
                                Get Plus+
                            </div>
                        </div>
