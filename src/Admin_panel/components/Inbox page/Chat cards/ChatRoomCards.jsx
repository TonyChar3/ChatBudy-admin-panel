...
const ChatRoomCards = ({ open_chat_function, visitor_name, visitor_id, unread_chat_count }) => {
    ...
    return(
        <>
            <div className="relative p-2 m-4 lg:p-3 w-[95%] md:w-4/6 lg:w-5/6 flex flex-row justify-between items-center bg-white border-[1px] border-[#6C2E9C] rounded-3xl shadow-custom-shadow-input">
                ...
                <div className={`relative ${show_full_name? 'w-full' : 'w-1/2'} p-2 ml-2`}>
                ...
                <div className={`${show_full_name && isNameTruncated? 'hidden' : ''} w-1/3 flex flex-row justify-around items-center`}>
                    <i onClick={OpenChat} className="fa-regular fa-comment relative text-4xl active:scale-[0.90] duration-100 ease-in hidden lg:inline text-[#A881D4] cursor-pointer"></i>
                    ...
                    <span className={`${unread_chat_count === 0? 'hidden' : 'bg-red-500 text-white'} absolute right-3 w-[25px] h-auto text-center rounded-full`}>{unread_chat_count}</span>
                </div>
            </div>
