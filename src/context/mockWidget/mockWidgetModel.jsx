                ...
                <div className="relative bg-[#e4e7ed] bg-opacity-[70%] w-[305px] h-[520px] rounded-lg">
                    {
                        ...
                        (
                            <>
                                <div className={`absolute w-full h-full transition-transform duration-200 ease-in-out boxShadow-custom-shadow-widget-content custom-scrollbar shadow-lg shadow-black rounded-lg ${widget_state? '' : 'transform-all scale-[0]'}`}>
                                    <div style={{ backgroundColor: main_color }} className={`p-2 ${font_color === 'light'? 'text-white' : 'text-zinc-700'} boxShadow-custom-shadow-widget-header border-b-[1px] border-transparent rounded-t-lg`}>
                                            ...
                                            </div>
                                            {
                                                ...
                                                (
                                                    chat_array.map((chat, i) => (
                                                        <div key={i} style={{ backgroundColor: main_color }} className={`inline-block max-w-[50%] h-auto m-2 rounded-t-lg rounded-bl-lg justify-self-end self-end rounded-t-lg rounded-bl-lg ${font_color === 'light'? 'text-white' : 'text-zinc-700'}`}>
                                ...
                                <div onClick={() => handleButtonClick()} style={{ backgroundColor: main_color }} className={`absolute ${widget_state? 'left-auto right-[20px]' : ''} ${position === 'right'? 'right-[20px] bottom-[2px]' : 'left-[20px] bottom-[2px]'} w-[50px] h-[50px] flex flex-row justify-center items-center ${shape === 'square'? 'rounded-lg' : 'rounded-full'} shadow-md shadow-black active:scale-[0.90] ease-in-out duration-300 cursor-pointer ${widget_state? 'z-5' : 'z-5'} transition-all ease duration-700`}>
                                    <i className={`${widget_state? 'fa-sharp fa-light fa-paper-plane-top' : 'fa-regular fa-messages-question'} ${font_color === 'light' ? 'text-white' : 'text-zinc-700'} text-white text-2xl`}></i>
                                </div>
                            </>
                        )
                        :
