        ...
        <>
            ...
            >
                <div className={`w-full flex flex-row justify-around items-center p-2 ${modal_error_mode? 'text-red-500' : ''}`}>
                    <i onClick={() => setModalOpen(false)} className={`hidden lg:block fa-solid fa-xmark mx-2 text-xl cursor-pointer ${modal_error_mode? 'text-[#E94E77]' : 'text-[#A881D4]'}`}></i>
                    <span className="text-lg p-1 mx-2">{modal_message}</span>
                    <i onClick={() => setModalOpen(false)} className={`fa-solid fa-xmark mx-2 text-xl cursor-pointer lg:hidden ${modal_error_mode? 'text-[#E94E77]' : 'text-[#A881D4]'}`}></i>
                </div>
            </div>
        </>
    ) : null;
