...
const NotificationCard = ({
  ...
  return (
    <>
      <div
        onClick={() => handleUserInteraction(notif_id)}
        className={`p-2 bg-[#A881D4] rounded-lg text-white transition-transform ${
          card_clicked
            ? "absolute w-full h-full z-50 top-0 left-0 bg-white bg-opacity-100 mx-0 my-0"
            : "w-[95%] mt-4 lg:my-4 mx-auto cursor-pointer"
        } duration-500 ease`}
      >
        <div>
          <p
            className={`font-normal my-1 lg:text-lg ${
              card_clicked ? "p-1 text-2xl" : "text-md"
            }`}
          >
            {notif_title}
          </p>
        ...
        {card_clicked ? (
          ...
          >
            ...
            <div className="my-4 w-full">
              <h3 className=" text-sm my-1 text-[#c0c2c4]">
                from: {sent_from}
              </h3>
              <h3 className="text-sm text-[#c0c2c4]">
                sent: {notif_sent_time}
              </h3>
              <div className="my-2 p-2 w-full flex flex-row justify-around items-center">
                {notif_action ? (
                  <i
                    onClick={() => handleFastReply()}
                    className="fa-sharp fa-light fa-reply mr-4 text-[#A881D4] text-2xl cursor-pointer"
                  ></i>
                ) : (
                  ""
                )}
                <i
                  ...
                  className="fa-solid fa-xmark-large text-[#f21f23] text-xl cursor-pointer"
                ></i>
              </div>
            </div>
          </motion.div>
