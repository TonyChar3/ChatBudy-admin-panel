...
const InboxSection = () => {
  ...
  const { desktop_chat_room } = AppState();
  ...
  useEffect(() => {
    if (Array.isArray(visitors_array) && Array.isArray(notification_array)) {
      const live_chat_visitors = visitors_array.filter(
        (visitor) => visitor.mode === "live-chat"
      );
                ...
                <div className="w-full flex flex-col justify-center lg:p-5 lg:justify-start items-center">
                  {inbox_visitor_array.length ? (
                    inbox_visitor_array.map((visitors, i) => (
                      <ChatRoomCards
                        key={i}
                        clean_up_notif_function={cleanUpUnreadChatCount}
                        open_chat_function={handleOpenChat}
                        visitor_name={visitors.email}
                        visitor_id={visitors._id}
                        unread_chat_count={visitors.unread_chat_count}
                      />
                    ))
                  ) : (
                    <>
