...
const AccountSection = ({ close_page_desktop }) => {
  const { user, setShowLoader, user_current_plan } = UserAuth();
  ...
  const SaveUpdatedInfo = async (e) => {
    ...
    // save the new data
    const saving = await saveNewProfileInfo(
      profile_info.user_name || user.displayName,
      profile_info.user_email || user.email,
      user.accessToken
    );
    if (saving.error) {
      setModalOpen(true);
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className={`${
            user.email === "randomprojectemail395@gmail.com" ? "hidden" : ""
          } absolute top-0 right-0 w-[80px] p-3 m-3 bg-[#A881D4] text-[#6C2E9C] rounded-3xl active:scale-[0.90] ease duration-300`}
        >
          <i className="fa-solid fa-user-minus text-xl"></i>
        </button>
        <form
          onSubmit={SaveUpdatedInfo}
          className="w-[90%] md:w-[65%] lg:w-[70%] xl:w-[65%] p-1 border-[1px] border-[#6C2E9C] bg-white rounded-xl shadow-custom-shadow-input"
        >
          ...
          <div
            className={`w-full flex flex-row justify-center items-center mt-3`}
          >
            {user.email === "randomprojectemail395@gmail.com" ? (
              <button
                ...
                className={`${
                  editMode ? "hidden" : ""
                } w-[35%] lg:w-[30%] p-1 m-1 text-lg text-white bg-[#6C2E9C] rounded-xl active:scale-[0.90] duration-200 ease-in-out`}
              >
                Edit
              </button>
            ) : (
              <button
                onClick={() => setMode((editMode) => !editMode)}
                type="button"
                className={`${
                  editMode ? "hidden" : ""
                } w-[35%] lg:w-[30%] p-1 m-1 text-lg text-white bg-[#6C2E9C] rounded-xl active:scale-[0.90] duration-200 ease-in-out`}
            ...
            <button
              ...
              className={`${
                editMode ? "" : "hidden"
              } w-[30%] lg:w-[15%] p-1 m-1 text-md text-white bg-[#19e392] rounded-xl active:scale-[0.90] duration-200 ease-in-out`}
            >
              Save
            </button>
            <button
              ...
              className={`${
                editMode ? "" : "hidden"
              } w-[30%] lg:w-[15%] p-1 m-1 text-md text-white bg-[#f53722] rounded-xl active:scale-[0.90] duration-200 ease-in-out`}
            >
              Cancel
            </button>
          </div>
          <div
            className={`${
              editMode ? "hidden" : ""
            } w-full flex justify-center text-sm text-[#A881D4] underline p-2`}
          >
            {user.email === "randomprojectemail395@gmail.com" ? (
              <h3 className="cursor-pointer">Update Password ?</h3>
            ) : (
              <h3
                onClick={() => setPasswordAuthModalOpen(true)}
          ...
          <div
            className={`${editMode ? "hidden" : ""} w-full flex justify-center`}
          >
            {user.emailVerified ? (
        ...
        <div
          className={`${
            user.email === "randomprojectemail395@gmail.com" ? "hidden" : ""
          } w-[90%] lg:w-[68%] p-2 lg:p-3 flex flex-row justify-start items-center text-[#A881D4]`}
        >
          {user_current_plan === "plus" ? (
            <>
              <div
                onClick={StartPortalSession}
                className="w-[45%] lg:w-[35%] bg-white p-2 border-[1px] border-[#6C2E9C] text-center rounded-xl cursor-pointer active:scale-[0.90] transform-all ease duration-300"
              >
                Manage billing
              </div>
            </>
